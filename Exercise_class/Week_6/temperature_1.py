...
num_coldest_days = int(input("Enter the number of coldest days to retrieve: "))
...
while end_day < start_day:
    print('Error: End day less than start day.')
    start_day = int(input("Enter the start day for average temperature calculation (1-30): "))
    end_day = int(input("Enter the end day for average temperature calculation (1-30): "))

week_remain = len(temperatures) % 7
# Extract weekly temperatures
start = (week_number - 1) * 7
end = week_number * 7
if week_number == 4:
    weekly_temperatures = temperatures[start:end + week_remain]
else:
...
coldest_days = sorted(temperatures)[:num_coldest_days]
print(f"The top {num_coldest_days} hottest days: {coldest_days}")
