...
def headless_run_karel_program(world_file=None):
    ...
    if world_file is None:
        ...
        if os.path.exists(possible):
            ...
            world_file = "worlds/FillBottomRow.w"
...
def grade(student_file):
    """Grade FillBottomRow solutions.

    Expectations (for every world matching worlds/FillBottomRow*.w):
      - After execution, every corner on street 1 from avenue 1 through num_avenues has exactly one beeper.
      - No extra beepers exist elsewhere (total beepers == num_avenues).
      - Karel ends at (num_avenues, 1) facing east (i.e., at the wall on the bottom row).
    """
    global CAPTURED_WORLD

    world_files = sorted(glob.glob("worlds/FillBottomRow*.w"))
    if not world_files:
        world_files = ["worlds/FillBottomRow.w"]
    ...
    for wf in world_files:
        ...
        # Verify beepers on bottom row
        beepers = world.beepers
        missing = []
        wrong_counts = []
        for a in range(1, width + 1):
            cnt = beepers.get((a, 1), 0)
            if cnt != 1:
                wrong_counts.append(((a, 1), cnt))
        total_beepers = sum(count for count in beepers.values() if count > 0)

        if wrong_counts:
            failures.append(f"{os.path.basename(wf)}: Expected exactly 1 beeper at each bottom corner (1..{width}), found mismatches: {wrong_counts}.")
            continue

        if total_beepers != width:
            failures.append(f"{os.path.basename(wf)}: Expected total beepers == {width}, found {total_beepers}.")
            continue
    ...
    return 100, ["Correct: bottom row filled with one beeper per corner and Karel at the eastern wall facing east for all FillBottomRow worlds"]