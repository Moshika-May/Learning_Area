...
def grade(student_file):
    """Grade Obstacles solutions.

    Expectations:
      - Beepers placed at (3,1), (4,1), (5,1) with exactly one beeper each and no other beepers remain.
      - Karel ends at (7,1) facing east.
    """
    global CAPTURED_WORLD
    CAPTURED_WORLD = None
    ...
    # Check beepers at target locations
    beepers = world.beepers
    b_3_1 = beepers.get((3, 1), 0)
    b_4_1 = beepers.get((4, 1), 0)
    b_5_1 = beepers.get((5, 1), 0)
    total_beepers = sum(count for count in beepers.values() if count > 0)

    if b_3_1 != 1:
        score = 0
        feedback.append(f"Expected 1 beeper at (3,1), found {b_3_1}.")

    if b_4_1 != 1:
        score = 0
        feedback.append(f"Expected 1 beeper at (4,1), found {b_4_1}.")

    if b_5_1 != 1:
        score = 0
        feedback.append(f"Expected 1 beeper at (5,1), found {b_5_1}.")

    if total_beepers != 3:
        score = 0
        feedback.append(f"Unexpected beeper total in the world (total={total_beepers}).")

    # Check Karel final position (avenue=7, street=1) and orientation (facing east)
    k = getattr(world, 'karel', None)
    if k is None:
        score = 0
        feedback.append("Could not find Karel instance to check final position and orientation.")
    else:
        if (k.avenue, k.street) != (7, 1):
            score = 0
            feedback.append(f"Karel final position expected (7,1), found ({k.avenue},{k.street}).")
        if not k.facing_east():
            score = 0
            feedback.append("Karel must be facing east at the end.")

    if score == 100:
        return 100, ["Correct: beepers placed at (3,1),(4,1),(5,1) and Karel at (7,1) facing east"]
