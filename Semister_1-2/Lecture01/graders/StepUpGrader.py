...
def grade(student_file):
    ...
    # Check beeper relocation: removed from (2,1) and placed at (4,2); exactly one beeper should remain
    beepers = world.beepers
    beeper_count_at_2_1 = beepers.get((2, 1), 0)
    beeper_count_at_4_2 = beepers.get((4, 2), 0)
    total_beepers = sum(count for count in beepers.values() if count > 0)

    if beeper_count_at_2_1 != 0:
        score = 0
        feedback.append("Karel did not pick up the beeper at (2,1).")

    if beeper_count_at_4_2 != 1:
        score = 0
        feedback.append(f"Beeper not placed at (4,2) (found {beeper_count_at_4_2}).")

    if total_beepers != 1:
        score = 0
        feedback.append(f"Unexpected beepers in the world (total={total_beepers}).")

    # Check Karel final position (avenue=5, street=2) and orientation (facing east)
    k = getattr(world, 'karel', None)
    if k is None:
        score = 0
        feedback.append("Could not find Karel instance to check final position and orientation.")
    else:
        if (k.avenue, k.street) != (5, 2):
            score = 0
            feedback.append(f"Karel final position expected (5,2), found ({k.avenue},{k.street}).")
        if not k.facing_east():
            score = 0
            feedback.append("Karel must be facing east at the end.")

    if score == 100:
        return 100, ["Correct: beeper at (4,2) and Karel at (5,2) facing east"]
